...
class TrainDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        batch_segms = torch.zeros(
           #  self.batch_per_gpu, this_short_size // self.segm_downsampling_rate, \
           # this_short_size // self.segm_downsampling_rate).long()
            self.batch_per_gpu, this_short_size, \
           this_short_size ).long()
        ...
        for i in range(self.batch_per_gpu):
            ...
            img = cv2.resize(img, (this_short_size, this_short_size), interpolation=cv2.INTER_LINEAR)
            segm = cv2.resize(segm, (this_short_size, this_short_size), interpolation=cv2.INTER_NEAREST)
            # print("dvdvds", img.shape)
...
class TestDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        for this_short_size in self.imgSize:
            ...
            #             self.imgMaxSize / float(max(ori_height, ori_width)))
            # target_height, target_width = int(ori_height * scale), int(ori_width * scale)
            # print(this_short_size, target_height, target_width)
            # # to avoid rounding in network
            # target_height = self.round2nearest_multiple(target_height, self.padding_constant)
            # target_width = self.round2nearest_multiple(target_width, self.padding_constant)

            # resize
            img_resized = cv2.resize(img.copy(), (this_short_size, this_short_size))
